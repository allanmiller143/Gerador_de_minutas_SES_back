...
def list_processos():
    ...
    for processo in processos:
        processo_data = {
            ...
            "dataRecebimento": processo.dataRecebimento.isoformat() if processo.dataRecebimento else None,
            "prioridade": processo.prioridade,
            "resumo": processo.resumo,
            "partes": processo.partes,
            "iaConfidence": processo.iaConfidence,
            "arquivoPdf": processo.arquivoPdf,
            "analista": processo.analista,
            "dataRevisao": processo.dataRevisao.isoformat() if processo.dataRevisao else None,
            "dataPreAnalise": processo.dataPreAnalise.isoformat() if processo.dataPreAnalise else None,
            "iaSugestao": processo.iaSugestao,
            "jurisprudenciasSugeridas": processo.jurisprudenciasSugeridas
        }
        output.append(processo_data)
...
def upload_processo():
    ...
    return jsonify({
        ...
        "processo": {
            "id": novo_processo.id,
            "numero": novo_processo.numero,
            "assunto": novo_processo.assunto,
            "status": novo_processo.status,
            "prioridade": novo_processo.prioridade,
            "arquivoPdf": novo_processo.arquivoPdf,
        }
    }), 201
...
def download_knowledge_base_file():
    ...
    if not file_path.startswith(knowledge_base_dir):
        # Se for apenas o nome do arquivo, ex: "PCDT_Doenca.pdf", prependemos o diretório da base
        # Mas se for uma tentativa de path traversal (ex: "../"), rejeitamos por segurança
